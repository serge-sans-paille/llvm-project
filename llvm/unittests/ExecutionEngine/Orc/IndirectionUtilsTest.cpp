...
TEST(IndirectionUtilsTest, MakeStub) {
  ...
  AttributeSet FnAttrs = AttributeSet::get(
      Context, SmallAttrBuilder(Context).addAttribute(Attribute::NoUnwind));
  AttributeSet RetAttrs; // None
  AttributeSet ArgAttrs[2] = {
      AttributeSet::get(Context, SmallAttrBuilder(Context).addStructRetAttr(ArgTy)),
      AttributeSet::get(Context, SmallAttrBuilder(Context).addByValAttr(ArgTy)),
  };
