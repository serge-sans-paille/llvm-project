...
GlobalVariable *makeObjectSizeHolder(
    Module &M, uint64_t MaxAlign,
    SmallVectorImpl<AllocaInst *> const &AllStaticAllocaInsts) {
  ...
  for (auto *AI : AllStaticAllocaInsts) {
    ...
    UIntTy ObjectSize =
        ((AllocationSizeInBits + MaxAlign - 1) / MaxAlign) * MaxAlign;
    ObjSizes.push_back(ObjectSize);
  }

  auto *GVObjSizes = new GlobalVariable(
      M,
      ArrayType::get(IntegerType::get(M.getContext(), 8 * sizeof(UIntTy)),
                     AllStaticAllocaInsts.size()),
...
bool runStackObjectCanary(Function &F, AliasAnalysis &AA, TargetMachine &TM,
  ...
  if (!PRNGState) {
    PRNGState = new GlobalVariable(
        M, IntegerType::get(F.getContext(), 64), false,
  ...
  auto *GVRange =
      new GlobalVariable(M,
                         ArrayType::get(IntegerType::get(F.getContext(), 64),
                                        AllStaticAllocaInsts.size()),
  ...
  if (MaxObjectSize <= std::numeric_limits<uint8_t>::max())
    GVObjSizes =
        makeObjectSizeHolder<uint8_t>(M, MaxAlign, AllStaticAllocaInsts);
  else if (MaxObjectSize <= std::numeric_limits<uint16_t>::max())
    GVObjSizes =
        makeObjectSizeHolder<uint16_t>(M, MaxAlign, AllStaticAllocaInsts);
  else if (MaxObjectSize <= std::numeric_limits<int32_t>::max())
    GVObjSizes =
        makeObjectSizeHolder<uint32_t>(M, MaxAlign, AllStaticAllocaInsts);
  else if (MaxObjectSize <= std::numeric_limits<int64_t>::max())
    GVObjSizes =
        makeObjectSizeHolder<uint64_t>(M, MaxAlign, AllStaticAllocaInsts);
  else
    assert(false);

  IntegerType *GVElementTy = cast<IntegerType>(
      cast<ArrayType>(
          cast<PointerType>(GVObjSizes->getType())->getElementType())
          ->getElementType());

  auto *GV = new GlobalVariable(
      M, ArrayType::get(GVElementTy, AllStaticAllocaInsts.size()), false,
      GlobalVariable::InternalLinkage,
  ...
  auto *PermInit =
      Function::Create(FunctionType::get(Type::getVoidTy(M.getContext()), {}),
                       GlobalVariable::InternalLinkage, "update_perm", M);
  ...
  for (uint64_t i = 0; i < AllStaticAllocaInsts.size(); ++i) {
    Value *Index = nullptr;
    Value *TargetPtr = PermBuilder.CreateInBoundsGEP(
        nullptr, GVRange,
        {ConstantInt::get(IntegerType::get(F.getContext(), 64), 0),
    ...
    if (i != AllStaticAllocaInsts.size() - 1) {
      Value *SwapIndex = PermBuilder.CreateAdd(
          ConstantInt::get(IntegerType::get(F.getContext(), 64), i),
          PermBuilder.CreateURem(
              CurrState, ConstantInt::get(IntegerType::get(F.getContext(), 64),
                                          AllStaticAllocaInsts.size() - i)));

      Value *SwapPtr = PermBuilder.CreateInBoundsGEP(
          nullptr, GVRange,
          {ConstantInt::get(IntegerType::get(F.getContext(), 64), 0),
           SwapIndex});
      ...
      CurrState = PermBuilder.CreateAdd(
          ConstantInt::get(IntegerType::get(F.getContext(), 64),
                           1442695040888963407ULL),
          PermBuilder.CreateMul(
              CurrState, ConstantInt::get(IntegerType::get(F.getContext(), 64),
    ...
    PermBuilder.CreateStore(
        Acc, PermBuilder.CreateInBoundsGEP(
                 nullptr, GV,
                 {ConstantInt::get(IntegerType::get(F.getContext(), 64), 0),
                  Index}));
    if (i != AllStaticAllocaInsts.size() - 1) {
      Value *ObjectSize = PermBuilder.CreateLoad(PermBuilder.CreateInBoundsGEP(
          nullptr, GVObjSizes,
          {ConstantInt::get(IntegerType::get(F.getContext(), 64), 0), Index}));
      if (i == 0)
        Acc = ObjectSize;
      else
  ...
  AllocaInst *StackEmulation = Builder.CreateAlloca(
      IntegerType::get(F.getContext(), 8),
      ConstantInt::get(IntegerType::get(F.getContext(), 64), CumulatedSize));
  StackEmulation->setAlignment(MaxAlign);

  uint64_t index = 0;
  for (auto *AI : AllStaticAllocaInsts) {
    Value *NewAI = Builder.CreateBitCast(
        Builder.CreateInBoundsGEP(
            nullptr, StackEmulation,
