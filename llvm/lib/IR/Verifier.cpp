...
#include "llvm/IR/Metadata.def"
  ...
  void verifyTailCCMustTailAttrs(const SmallAttrBuilder &Attrs, StringRef Context);
...
void Verifier::verifyParameterAttrs(AttributeSet Attrs, Type *Ty,
  ...
  SmallAttrBuilder IncompatibleAttrs = AttributeFuncs::typeIncompatible2(Ty);
  for (Attribute Attr : Attrs) {
...
void Verifier::verifyTailCCMustTailAttrs(const SmallAttrBuilder & Attrs,
                                         StringRef Context) {
...
static SmallAttrBuilder getParameterABIAttributes(LLVMContext& Ctx, unsigned I, AttributeList Attrs) {
...
void Verifier::verifyMustTailCall(CallInst &CI) {
  ...
  if (CI.getCallingConv() == CallingConv::SwiftTail ||
    ...
    for (unsigned I = 0, E = CallerTy->getNumParams(); I != E; ++I) {
      SmallAttrBuilder ABIAttrs = getParameterABIAttributes(F->getContext(), I, CallerAttrs);
    ...
    for (unsigned I = 0, E = CalleeTy->getNumParams(); I != E; ++I) {
      SmallAttrBuilder ABIAttrs = getParameterABIAttributes(F->getContext(), I, CalleeAttrs);
  ...
  for (unsigned I = 0, E = CallerTy->getNumParams(); I != E; ++I) {
    SmallAttrBuilder CallerABIAttrs = getParameterABIAttributes(F->getContext(), I, CallerAttrs);
    SmallAttrBuilder CalleeABIAttrs = getParameterABIAttributes(F->getContext(), I, CalleeAttrs);
